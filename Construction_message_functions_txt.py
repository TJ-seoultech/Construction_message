...
def making_text(tem_p, hum_p, rain_p, message_list, checkBox_object, num):
    if checkBox_object.isChecked() == True:
        if tem_p > 30:
            ...
            f.write("실외 공사는 가급적이면 쉬어주세요!" + "\n")
        elif rain_p > 0:
            message_list.append(["비가 예상됩니다. 비가 온다면 바로 실외 공사를 중단해주세요!\n"])
            f.write("비가 예상됩니다. 비가 온다면 바로 실외 공사를 중단해주세요!" + "\n")
        else:
              pass
    ...
    if num == 1:
        ...
        if checkBox_object.isChecked()==True:
            ...
            f.write("1. 철근은 도면에 따라 바르게 배근하고 콘크리트 타설 완료시까지 움직이지 않도록 견고하게 조립해야 됩니다!" + "\n" + "2. 슬래브 배근시 작업통로 및 발판 설치로 철근 변형이나 유효높이 미확보 사례를 방지 해주세요!" + "\n" + "3. 고임대 및 간격재는 콘크리트제 및 플라스틱제를 사용하며 사용 전 KS인증을 받은 규격품 확인이 필요합니다!" + "\n")
    elif num == 5:     #콘크리트공사
        if checkBox_object.isChecked()==True:
            if tem_p <= 4:
                message_list.append(["한중콘크리트로 시공을 해주세요.", "모르타르 작업을 할 수 없는 환경입니다.", "1. 방수 및 고정처리가 절실히 필요한 부위는 타설 전 반드시 확인해야 합니다!", "2. 지하 외벽으로부터 내부로 관통하는 각종 슬리브는 누수방지용 설치여부를 반드시 확인해야 합니다!",
                                     "3. 콘크리트 타설 시 철근 및 매설물의 배치나 거푸집이 변형 손상되지 않도록 주의해야 합니다!"])
                f.write("한중콘크리트로 시공을 해주세요." + "\n" + "모르타르 작업을 할 수 없는 환경입니다." + "\n" + "1. 방수 및 고정처리가 절실히 필요한 부위는 타설 전 반드시 확인해야 합니다!" + "\n" + "2. 지하 외벽으로부터 내부로 관통하는 각종 슬리브는 누수방지용 설치여부를 반드시 확인해야 합니다!" + "\n" +
              "3. 콘크리트 타설 시 철근 및 매설물의 배치나 거푸집이 변형 손상되지 않도록 주의해야 합니다!" + "\n")
            if tem_p > 25:
                ...
                f.write("서중콘크리트로 시공을 해주세요." + "\n" + "1. 방수 및 고정처리가 절실히 필요한 부위는 타설 전 반드시 확인해야 합니다!" + "\n" + "2. 지하 외벽으로부터 내부로 관통하는 각종 슬리브는 누수방지용 설치여부를 반드시 확인해야 합니다!" + "\n" + "3. 콘크리트 타설 시 철근 및 매설물의 배치나 거푸집이 변형 손상되지 않도록 주의해야 합니다!" + "\n")
            if tem_p < -10:
                message_list.append(["콘크리트 치기 작업을 할 수 없는 환경입니다."])
                f.write("콘크리트 치기 작업을 할 수 없는 환경입니다." + "\n")
